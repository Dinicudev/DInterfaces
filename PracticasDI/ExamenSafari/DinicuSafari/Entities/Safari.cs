...
namespace DinicuSafari.Entities
{
    ...
    public class Safari
    {
        ...
        private int turnoActual; // Lleva el conteo del turno actual.Examen3

        public bool EsNoche => turnoActual % 3 == 0; // Cada tercer turno es noche.Examen3
        ...
        public string[,] ObtenerEstado()
        {
            ...
            foreach (var ser in listaDeSeres)
            {
                if (ser is Planta)
                    ...
                    tablero[ser.PosX, ser.PosY] = "Gacela"; // Si es una gacela, lo marcamos como gacela.
                else if (ser is Elefante)
                    tablero[ser.PosX, ser.PosY] = "Elefante"; // Si es un elefante, lo marcamos como elefante.//Examen1
        ...
        public void EjecutarTurno()
        {
            Console.WriteLine(EsNoche ? "---- Es de Noche ----" : "---- Es de Día ----");

            foreach (var ser in new List<Ser>(listaDeSeres))
            {
                ser.Edad++; // Incrementamos la edad de todos los seres.

                if (ser is Animal animal)
                {
                    Console.WriteLine($"{animal.Nombre} (Edad: {animal.Edad}) está en ({animal.PosX}, {animal.PosY}).");

                    if (!EsNoche) // Si es de día, los animales pueden moverse y comer.
                    {
                        MoverAnimal(animal); // Movemos al animal.
                    ...
                    // Los animales pueden reproducirse siempre, incluso de noche.
                    if (animal.Edad % animal.TurnosParaReproducirse == 0)
                    {
                ...
                else if (ser is Planta planta)
                {
                    if (planta.Edad % 2 == 0) // Las plantas se reproducen cada 2 turnos.
                    {
                        ReproducirPlanta(planta);
                        Console.WriteLine($"{planta.Nombre} se ha reproducido.");
                    }
                }
            }

            turnoActual++; // Incrementamos el turno al final.
            Console.WriteLine("---- Fin del Turno ----");
        }
        ...
        //Vamos a crear el metodo ejecutar 10 pasos turno Examen2
        public void Ejecutar10Turnos()
        {
            for (int i = 0; i < 10; i++)
            {
                EjecutarTurno(); // Ejecuta un turno completo.
            }
        }
